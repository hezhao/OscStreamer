...
This program connects to a NatNet server, receives a data stream, and broadcast that data steram
to a LocalIP default to local machine, and then redirects data to client(s) on the network 
[optinal: writes data stream to an ascii file].
...
int CreateClient(int iConnectionType)
{
    ...
    else
    {
        ...
        printf("[NatNetOSC] Server application info:\n");
